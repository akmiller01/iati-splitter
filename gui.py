...
class Window:
    def __init__(self, master):
        self.input = None
        self.output = None
        Label(root, text="Target filesize (MB)").grid(row=1, column=0, sticky=W)
        Label(root, text="XML file").grid(row=2, column=0, sticky=W)
        Label(root, text="Output directory (optional)").grid(row=3, column=0, sticky=W)
        self.bart = Entry(master)
        self.bart.grid(row=1, column=1, sticky=W + E)
        self.bari = Entry(master, state='disabled')
        self.bari.grid(row=2, column=1, sticky=W + E)
        self.baro = Entry(master, state='disabled')
        self.baro.grid(row=3, column=1, sticky=W + E)

        # Buttons
        self.cbutton = Button(root, text="Split XML", command=self.process)
        self.cbutton.grid(row=4, column=3, sticky=E)
        self.ibutton = Button(root, text="Browse", command=self.browseinput)
        self.ibutton.grid(row=2, column=3, sticky=E)
    ...
    def browseinput(self):
        Tk().withdraw()
        self.input = filedialog.askopenfilename(filetypes=[('XML files', '.xml'), ('All files', '.*')])
    ...
    def process(self):
        if self.input:
            split_iati(float(self.bart.get()), self.input, self.output)
        else:
            print("Error: Please select one input XML file.")
...
root = Tk()
root.title("DevInit IATI XML splitter")
